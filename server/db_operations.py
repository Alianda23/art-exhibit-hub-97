...
def get_user_tickets(user_id):
    """Get tickets for a specific user from database"""
    connection = get_db_connection()
    if connection is None:
        return {"error": "Database connection failed"}
    ...
    try:
        ...
        query = """
        SELECT t.*, u.name as user_name, e.title as exhibition_title, e.location
        FROM tickets t
        JOIN users u ON t.user_id = u.id
        JOIN exhibitions e ON t.exhibition_id = e.id
        WHERE t.user_id = %s
...
def get_user_orders(user_id):
    """Get orders for a specific user from database"""
    connection = get_db_connection()
    if connection is None:
        return {"error": "Database connection failed"}
    ...
    try:
        ...
        SELECT o.*, u.name as user_name,
               ...
               END as item_title,
               CASE 
                   WHEN o.type = 'artwork' THEN a.artist
                   WHEN o.type = 'exhibition' THEN NULL
               END as artist
        FROM orders o
        JOIN users u ON o.user_id = u.id
        LEFT JOIN artworks a ON o.type = 'artwork' AND o.reference_id = a.id
